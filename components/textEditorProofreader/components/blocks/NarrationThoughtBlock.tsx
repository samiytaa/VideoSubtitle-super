...
const NarrationThoughtBlockItem = React.memo(({ block, index, blockKey }: BasicBlockItemProps<Extract<ParsedBlock, { type: 'narration-thought' }>>) => {
  ...
  return (
    <div
      ...
      className={`group relative z-0 overflow-visible px-4 py-3 rounded-lg text-sm leading-relaxed cursor-pointer transition-all hover:z-20 ${isMenuOpen ? 'z-30' : ''} ${isEditing ? 'ring-2 ring-indigo-500' : ''}`}
